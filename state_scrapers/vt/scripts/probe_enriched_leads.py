#!/usr/bin/env python3
"""Probe ME curated leads (with `pre_discovered_pdf_urls`) and bank into
gs://hoaproxy-bank/v1/ME/. The stock `python -m hoaware.discovery
probe-batch` constructs `Lead` from JSON and ignores extra keys, which drops
the `pre_discovered_pdf_urls` carried by curated/direct-PDF leads. This driver
keeps both.
"""
...
LEAD_FIELDS = {"name", "source", "source_url", "state", "city", "website", "county"}
...
def main() -> int:
    ...
    p.add_argument("--input", default=str(ROOT / "state_scrapers/vt/leads/vt_continuation_leads.jsonl"))
    p.add_argument("--output", default=str(ROOT / "state_scrapers/vt/leads/vt_continuation_probe_results.jsonl"))
