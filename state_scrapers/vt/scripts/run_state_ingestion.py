#!/usr/bin/env python3
"""Run Vermont HOA ingestion end-to-end.
...
Pipeline (per ``docs/multi-state-ingestion-playbook.md``):
  ...
  1. Discovery — three modes, selected via --discovery-mode:
       ...
       sos-first       Skip Serper discovery; expects a curated lead file at
                       leads/vt_sos_leads.jsonl (RI/CT/NH/ME/VT pattern
                       for small states where county/town names overlap other
                       states).
       manual-leads    Skip discovery entirely; expects
                       leads/vt_curated_leads.jsonl. Use when leads
                       are pre-collected from an open portal or aggregator.

  2. prepare_bank_for_ingest.py with --max-docai-cost-usd 5
     → prepared bundles in gs://hoaproxy-ingest-ready/v1/VT/.

  3. POST /admin/ingest-ready-gcs?state=VT in a loop until empty
...
## Placeholders to replace before use
  ...
  VT uses the SoS-first playbook branch when the Vermont registry is
  programmatically reachable. Current public registry access requires CAPTCHA
  and direct API calls are blocked by Imperva, so COUNTY_RUNS is populated for
  the playbook fallback: county-scoped deterministic Serper.
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    ("vt_chittenden_serper_queries.txt", "Chittenden"),
    ("vt_rutland_serper_queries.txt", "Rutland"),
    ("vt_washington_serper_queries.txt", "Washington"),
    ("vt_bennington_serper_queries.txt", "Bennington"),
    ("vt_lamoille_serper_queries.txt", "Lamoille"),
    ("vt_windsor_serper_queries.txt", "Windsor"),
    ("vt_windham_serper_queries.txt", "Windham"),
    ("vt_addison_serper_queries.txt", "Addison"),
    ("vt_franklin_serper_queries.txt", "Franklin"),
    ("vt_orleans_serper_queries.txt", "Orleans"),
    ("vt_caledonia_serper_queries.txt", "Caledonia"),
    ("vt_orange_serper_queries.txt", "Orange"),
    ("vt_grand_isle_serper_queries.txt", "Grand Isle"),
    ("vt_essex_serper_queries.txt", "Essex"),
]
...
def main() -> int:
    ...
    parser.add_argument("--max-docai-cost-usd", type=float, default=5.0,
