#!/usr/bin/env python3
"""Run Hawaii HOA ingestion end-to-end.
...
## Placeholders to replace before use
  ...
  HI             Two-letter state code, e.g. "MT"
  Hawaii        Full state name, e.g. "Montana"
  {"max_lat": 22.24, "max_lon": -154.75, "min_lat": 18.86, "min_lon": -160.27}        Dict with min_lat/max_lat/min_lon/max_lon for OOB check
...
STATE_BBOX: dict[str, float] = {"max_lat": 22.24, "max_lon": -154.75, "min_lat": 18.86, "min_lon": -160.27}
# ---------------------------------------------------------------------------
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    ("hi_honolulu_serper_queries.txt", "Honolulu"),
    ("hi_hawaii_serper_queries.txt", "Hawaii"),
    ("hi_maui_serper_queries.txt", "Maui"),
    ("hi_kauai_serper_queries.txt", "Kauai"),
    ("hi_host_family_serper_queries.txt", None),
]
