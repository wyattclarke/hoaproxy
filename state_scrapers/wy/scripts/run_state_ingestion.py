...
STATE = "WY"
STATE_NAME = "Wyoming"
# Wyoming is a near-perfect rectangle: 41.0–45.0 N, 104.05–111.05 W.
STATE_BBOX: dict[str, float] = {
    "min_lat": 40.99, "max_lat": 45.01,
    "min_lon": -111.06, "max_lon": -104.04,
}
# ---------------------------------------------------------------------------
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    # Teton: Jackson Hole resort condos — likely 50-70% of statewide HOAs.
    ("wy_teton_serper_queries.txt", "Teton"),
    # Laramie: Cheyenne front-range planned communities.
    ("wy_laramie_serper_queries.txt", "Laramie"),
    # Natrona: Casper.
    ("wy_natrona_serper_queries.txt", "Natrona"),
    # Park: Cody / Yellowstone gateway.
    ("wy_park_serper_queries.txt", "Park"),
    # Sublette: Pinedale resort condos.
    ("wy_sublette_serper_queries.txt", "Sublette"),
    # Sheridan: Sheridan, Big Horn foothills.
    ("wy_sheridan_serper_queries.txt", "Sheridan"),
    # Albany: Laramie city / university.
    ("wy_albany_serper_queries.txt", "Albany"),
    # Lincoln: Star Valley / Alpine — Jackson-adjacent resort condos.
    ("wy_lincoln_serper_queries.txt", "Lincoln"),
    # Fremont: Lander / Dubois / Wind River resort area.
    ("wy_fremont_serper_queries.txt", "Fremont"),
]
...
def main() -> int:
    ...
    parser.add_argument("--max-docai-cost-usd", type=float, default=5.0,
                        help="Hard cap on Google Document AI spend for this run (Tier 0)")
