#!/usr/bin/env python3
"""Run Alabama HOA ingestion end-to-end.
...
Pipeline (per ``docs/multi-state-ingestion-playbook.md``):
  ...
  1. Discovery — three modes, selected via --discovery-mode:
       ...
       sos-first       Skip Serper discovery; expects a curated lead file at
                       leads/AL_sos_leads.jsonl (RI/CT/NH/ME/VT pattern
                       for small states where county/town names overlap other
                       states).
       manual-leads    Skip discovery entirely; expects
                       leads/AL_curated_leads.jsonl. Use when leads
                       are pre-collected from an open portal or aggregator.

  2. prepare_bank_for_ingest.py with --max-docai-cost-usd 30
     → prepared bundles in gs://hoaproxy-ingest-ready/v1/AL/.

  3. POST /admin/ingest-ready-gcs?state=AL in a loop until empty
...
## Placeholders to replace before use
  ...
  AL             Two-letter state code, e.g. "MT"
  Alabama        Full state name, e.g. "Montana"
  {"max_lat": 35.01, "max_lon": -84.89, "min_lat": 30.14, "min_lon": -88.47}        Dict with min_lat/max_lat/min_lon/max_lon for OOB check
  1              Tier from playbook (0-4)
  30     Default OCR budget in USD (see Appendix D cost defaults)
...
# ---------------------------------------------------------------------------
STATE = "AL"           # e.g. "MT"
STATE_NAME = "Alabama" # e.g. "Montana"
...
STATE_BBOX: dict[str, float] = {"max_lat": 35.01, "max_lon": -84.89, "min_lat": 30.14, "min_lon": -88.47}
# ---------------------------------------------------------------------------
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    ("al_jefferson_serper_queries.txt", "Jefferson"),
    ("al_madison_serper_queries.txt", "Madison"),
    ("al_mobile_serper_queries.txt", "Mobile"),
    ("al_baldwin_serper_queries.txt", "Baldwin"),
    ("al_shelby_serper_queries.txt", "Shelby"),
    ("al_tuscaloosa_serper_queries.txt", "Tuscaloosa"),
    ("al_montgomery_serper_queries.txt", "Montgomery"),
    ("al_lee_serper_queries.txt", "Lee"),
    ("al_morgan_serper_queries.txt", "Morgan"),
    ("al_calhoun_serper_queries.txt", "Calhoun"),
    ("al_etowah_serper_queries.txt", "Etowah"),
    ("al_houston_serper_queries.txt", "Houston"),
    ("al_limestone_serper_queries.txt", "Limestone"),
    ("al_marshall_serper_queries.txt", "Marshall"),
    ("al_st-clair_serper_queries.txt", "St. Clair"),
    ("al_elmore_serper_queries.txt", "Elmore"),
    ("al_cullman_serper_queries.txt", "Cullman"),
    ("al_talladega_serper_queries.txt", "Talladega"),
    ("al_host_family_serper_queries.txt", None),
]
...
def main() -> int:
    ...
    parser.add_argument("--max-docai-cost-usd", type=float, default=30,
