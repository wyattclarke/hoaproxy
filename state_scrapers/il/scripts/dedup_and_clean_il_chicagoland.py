"""Phase 10 cleanup for IL Chicagoland live HOAs.
...
def _is_chicagoland_eligible(name: str, name_to_prefix: dict[str, str]) -> tuple[bool, str]:
    """Return (eligible, reason). Eligible = Chicagoland's territory only.

    The downstate session handles unmapped / recovery / cross-state-leak rows
    so this Chicagoland cleaner stays narrowly scoped to the seven explicit
    Chicagoland counties to avoid double-cleanup collisions."""
    county = _county_for(name, name_to_prefix)
    if county is None:
        return False, "unmapped"  # downstate session handles unmapped
    if county in CHICAGOLAND_COUNTIES:
        return True, f"chicagoland:{county}"
    return False, f"out-of-scope:{county}"
...
def main() -> int:
    ...
    p.add_argument("--out-dir",
                   default="state_scrapers/il/results/cleanup_chicagoland")
    ...
    for r in summaries:
        ok, why = _is_chicagoland_eligible(r.get("hoa") or "", name_to_prefix)
        if ok:
            eligible.append((r, why))
        else:
            excluded.append((r, why))
    print(f"eligible (Chicagoland scope): {len(eligible)} | excluded (downstate/unmapped): {len(excluded)}",
          file=sys.stderr)
