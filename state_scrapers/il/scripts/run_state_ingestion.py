#!/usr/bin/env python3
"""Run Illinois HOA ingestion end-to-end.
...
Pipeline (per ``docs/multi-state-ingestion-playbook.md``):
  ...
  1. Discovery — three modes, selected via --discovery-mode:
       ...
       sos-first       Skip Serper discovery; expects a curated lead file at
                       leads/IL_sos_leads.jsonl (RI/CT/NH/ME/VT pattern
                       for small states where county/town names overlap other
                       states).
       manual-leads    Skip discovery entirely; expects
                       leads/IL_curated_leads.jsonl. Use when leads
                       are pre-collected from an open portal or aggregator.

  2. prepare_bank_for_ingest.py with --max-docai-cost-usd 150
     → prepared bundles in gs://hoaproxy-ingest-ready/v1/IL/.

  3. POST /admin/ingest-ready-gcs?state=IL in a loop until empty
...
## Placeholders to replace before use
  ...
  IL             Two-letter state code, e.g. "MT"
  Illinois        Full state name, e.g. "Montana"
  {"max_lat": 42.51, "max_lon": -87.49, "min_lat": 36.97, "min_lon": -91.52}        Dict with min_lat/max_lat/min_lon/max_lon for OOB check
  3              Tier from playbook (0-4)
  150     Default OCR budget in USD (see Appendix D cost defaults)
...
# ---------------------------------------------------------------------------
STATE = "IL"           # e.g. "MT"
STATE_NAME = "Illinois" # e.g. "Montana"
...
STATE_BBOX: dict[str, float] = {"max_lat": 42.51, "max_lon": -87.49, "min_lat": 36.97, "min_lon": -91.52}
# ---------------------------------------------------------------------------
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    ("il_cook_serper_queries.txt", "Cook"),
    ("il_dupage_serper_queries.txt", "DuPage"),
    ("il_lake_serper_queries.txt", "Lake"),
    ("il_will_serper_queries.txt", "Will"),
    ("il_kane_serper_queries.txt", "Kane"),
    ("il_mchenry_serper_queries.txt", "McHenry"),
    ("il_kendall_serper_queries.txt", "Kendall"),
    ("il_winnebago_serper_queries.txt", "Winnebago"),
    ("il_sangamon_serper_queries.txt", "Sangamon"),
    ("il_champaign_serper_queries.txt", "Champaign"),
    ("il_peoria_serper_queries.txt", "Peoria"),
    ("il_mclean_serper_queries.txt", "McLean"),
    ("il_st-clair_serper_queries.txt", "St. Clair"),
    ("il_madison_serper_queries.txt", "Madison"),
    ...
    # Wave C remainder — 14 long-tail counties (session 3)
    ("il_vermilion_serper_queries.txt", "Vermilion"),
    ("il_knox_serper_queries.txt", "Knox"),
    ("il_effingham_serper_queries.txt", "Effingham"),
    ("il_coles_serper_queries.txt", "Coles"),
    ("il_marion_serper_queries.txt", "Marion"),
    ("il_boone_serper_queries.txt", "Boone"),
    ("il_grundy_serper_queries.txt", "Grundy"),
    ("il_stephenson_serper_queries.txt", "Stephenson"),
    ("il_whiteside_serper_queries.txt", "Whiteside"),
    ("il_henry_serper_queries.txt", "Henry"),
    ("il_lee_serper_queries.txt", "Lee"),
    ("il_ogle_serper_queries.txt", "Ogle"),
    ("il_jackson_serper_queries.txt", "Jackson"),
    ("il_franklin_serper_queries.txt", "Franklin"),
...
def main() -> int:
    ...
    parser.add_argument("--max-docai-cost-usd", type=float, default=150,
