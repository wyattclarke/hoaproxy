...
Usage:
    ...
    # Phase 1: pre-flight audit (no API calls). Writes counts to results/fl_here_audit.json.
    python state_scrapers/fl/scripts/fl_enrich_with_here.py --audit

    # Phase 2: 10-manifest smoke test (real HERE calls).
    python state_scrapers/fl/scripts/fl_enrich_with_here.py --smoke

    # Phase 3: full pass (DO NOT run until OSM finishes; user triggers).
    python state_scrapers/fl/scripts/fl_enrich_with_here.py --run [--limit N] [--rate-per-sec 4]
...
AUDIT_JSON = RESULTS_DIR / "fl_here_audit.json"
SMOKE_JSON = RESULTS_DIR / "fl_here_smoke.json"
...
def _build_query(addr: dict) -> str:
    ...
    state = (addr.get("state") or "FL").strip() or "FL"
...
def phase1_audit() -> None:
    ...
    eligible_list = AUDIT_JSON.with_name("fl_here_eligible_uris.txt")
...
def phase3_run(api_key: str, limit: int, rate_per_sec: float, dry_run: bool) -> None:
    ...
    ledger_path = RESULTS_DIR / f"fl_here_run_{ts}.jsonl"
    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
