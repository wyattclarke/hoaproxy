#!/usr/bin/env python3
"""Re-route 73 FL bank manifests flagged by OCR as belonging to another state.

Background: the FL OCR pass (``fl_bank_ocr_enrich.py run``) emits
``outcome: "state_mismatch"`` when a manifest's PDF text mentions a non-FL
...
For each flagged manifest:
  ...
  3. Apply a stricter detection: count "recorder-style" hits per state
     ...
     AND no recorder-style FL hit at all. Otherwise leave under FL with
     ``audit.ocr_state_mismatch.review_pending: true``.
  4. If unambiguous: copy the entire ``v1/FL/<county>/<slug>/`` tree to
     ...
     then delete the FL tree.

Idempotency: if the FL manifest already has ``audit.rerouted_to``, skip.
If the new prefix already has a manifest, log it and (only if the FL tree
still exists) delete the FL tree. The dest manifest is written first;
the FL tree is only deleted after the dest manifest blob is confirmed.

Audit log: ``state_scrapers/fl/results/fl_state_reroute_audit.jsonl`` with
...
Usage:
  ...
  python state_scrapers/fl/scripts/fl_reroute_state_mismatches.py \\
      [--dry-run] [--ledger PATH]
"""
...
log = logging.getLogger("fl_reroute")


BUCKET = os.environ.get("HOA_BANK_GCS_BUCKET", "hoaproxy-bank")
VERSION = "v1"
SOURCE_STATE = "FL"
DEFAULT_LEDGER = ROOT / "state_scrapers" / "fl" / "results" / "fl_state_reroute_audit.jsonl"
DEFAULT_OCR_RUN = ROOT / "state_scrapers" / "fl" / "results" / "fl_ocr_run_20260509T033205Z.jsonl"

# State name -> 2-letter abbreviation (no FL).
US_STATES_NON_FL: dict[str, str] = {
...
US_ABBR_TO_NAME: dict[str, str] = {abbr: name for name, abbr in US_STATES_NON_FL.items()}

# ---------------------------------------------------------------------------
# Detection: "recorder-style" non-FL evidence
# ---------------------------------------------------------------------------
...
COUNTY_STATE_NAME_RE = re.compile(
    ...
    + "|".join(re.escape(n) for n in US_STATES_NON_FL) + r")\b",
)
...
def detect_target_state(combined_text: str) -> tuple[str | None, dict[str, Any]]:
    ...
    Decision rules:
      ...
      * Else: the leading non-FL state with >=2 recorder-style hits wins.
      * Else: ambiguous.
    """
    ...
    for m in COUNTY_STATE_NAME_RE.finditer(head):
        name = m.group(1)
        abbr = US_STATES_NON_FL.get(name)
        if abbr:
...
def _patch_dest_manifest_state(
    ...
    *,
    ...
    # The old county slug came from FL; it does not apply to the new state.
...
def main() -> int:
    ...
    with ledger_path.open("w") as ledger_fh:
        for i, row in enumerate(flagged):
            old_blob = row["manifest"]  # e.g. v1/FL/brevard/quail-ridge/manifest.json
            ...
            if manifest is None:
                ...
                # successful reroute (we delete the FL tree at end of run).
            ...
            if target_abbr is None:
                # Mark review_pending and leave under FL.
