...
Usage:
  ...
  # Phase A
  python state_scrapers/fl/scripts/fl_bank_ocr_enrich.py audit \\
      --out data/fl_ocr_audit.json

  # Phase B (smoke)
  python state_scrapers/fl/scripts/fl_bank_ocr_enrich.py smoke \\
      --county sumter --limit 10

  # Phase C (full — requires explicit confirmation)
  python state_scrapers/fl/scripts/fl_bank_ocr_enrich.py run \\
      --max-cost-usd 396
...
log = logging.getLogger("fl_bank_ocr")
...
FL_BBOX = (24.4, 31.2, -87.7, -79.9)  # min_lat max_lat min_lon max_lon
...
ADDRESS_RE = re.compile(
    r"\b([A-Z][A-Za-z\s.'-]{2,30}),\s*(Florida|FL)\s+(\d{5}(?:-\d{4})?)\b",
    re.IGNORECASE,
)
# Capture the immediate token(s) before "County, Florida". We allow up
...
COUNTY_RE = re.compile(
    r"\b([A-Z][a-zA-Z]{2,20}(?:\s+[A-Z][a-zA-Z]{2,20})?)\s+County,\s+Florida\b",
)
# Match the longer "Of [Name] County, Florida" / "Records Of [Name] County, Florida"
# preamble so we can skip those captures.
COUNTY_BOILERPLATE_RE = re.compile(
    r"\b(?:public\s+)?records\s+of\s+(?:the\s+)?[A-Z][a-zA-Z]{2,20}\s+County,\s+Florida",
    re.IGNORECASE,
)
...
ZIP_RE = re.compile(r"\b(3[2-4]\d{3})(?:-\d{4})?\b")  # FL ZIPs: 32000-34999
...
FLORIDA_RE = re.compile(r"\bFlorida\b|\bFL\s+\d{5}\b", re.IGNORECASE)
...
def _sidecar_blob_name(doc: dict[str, Any]) -> str | None:
    ...
    if not gcs_path.startswith("gs://"):
        return None
    # gs://hoaproxy-bank/v1/FL/{county}/{slug}/doc-{sha}/original.pdf
    # sidecar goes at: v1/FL/{county}/{slug}/doc-{sha}/sidecar.json
...
def _extract_geo(text: str) -> dict[str, Any]:
    ...
    # Find first non-boilerplate "X County, Florida" mention.
    boilerplate_spans = [m.span() for m in COUNTY_BOILERPLATE_RE.finditer(head)]
    for m in COUNTY_RE.finditer(head):
        # Skip if this match falls inside a "records of X County, Florida" span.
...
def _detect_state_mismatch(text: str) -> tuple[str | None, str]:
    head = text[:3000]
    has_fl = bool(FLORIDA_RE.search(head))
    other_matches = OTHER_STATE_RE.findall(head)
    if other_matches and not has_fl:
        from collections import Counter
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    results_dir = ROOT / "state_scrapers" / "fl" / "results"
    ledger_path = results_dir / f"fl_ocr_run_{timestamp}.jsonl"
    ledger_path.parent.mkdir(parents=True, exist_ok=True)

    log.info("Full FL OCR pass. Budget: $%.2f. Ledger: %s", args.max_cost_usd, ledger_path)
    log.info("Scanning gs://%s/%s ...", args.bank_bucket, BANK_PREFIX)
    ...
    county_filter = args.county.lower().replace(" ", "-") if args.county else None
    ...
    for i, blob_name in enumerate(all_blobs):
        if i % 100 == 0:
            log.info("Progress: %d/%d manifests, cost=$%.4f", i, len(all_blobs), total_cost)
        ...
        for doc, cls, reason in selected:
            ...
            # Budget check for DocAI
            if cls == "docai_candidate":
                projected = pc * COST_DOCAI_PER_PAGE
                if total_cost + projected > args.max_cost_usd:
                    budget_deferred += 1
                    _append_ledger({
                        "manifest": blob_name,
                        "sha": sha[:16],
...
def main() -> int:
    ...
    p_audit.add_argument("--out", default="state_scrapers/fl/results/fl_ocr_audit.json")

    # smoke
    p_smoke = sub.add_parser("smoke", help="Phase B: smoke-test on N HOAs from one county")
    p_smoke.add_argument("--county", default="sumter", help="County slug to test")
    ...
    p_smoke.add_argument("--out", default="state_scrapers/fl/results/fl_ocr_smoke.json")

    # run
    p_run = sub.add_parser("run", help="Phase C: full pass (only after smoke approval)")
    p_run.add_argument("--max-cost-usd", type=float, default=396.0)
    p_run.add_argument("--county", default="", help="Optionally restrict to one county")
