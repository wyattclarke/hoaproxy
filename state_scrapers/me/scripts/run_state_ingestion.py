#!/usr/bin/env python3
"""Run Maine HOA ingestion end-to-end.
...
Pipeline (per ``docs/multi-state-ingestion-playbook.md``):
  ...
  1. Discovery — three modes, selected via --discovery-mode:
       ...
       sos-first       Skip Serper discovery; expects a curated lead file at
                       leads/me_sos_leads.jsonl (RI/CT/NH/ME/VT pattern
  ...
  2. prepare_bank_for_ingest.py with --max-docai-cost-usd 10
     → prepared bundles in gs://hoaproxy-ingest-ready/v1/ME/.

  3. POST /admin/ingest-ready-gcs?state=ME in a loop until empty
...
Maine is Tier 1. Appendix D recommends SoS-first, but the public ICRS
corporate search currently presents reCAPTCHA and an explicit automated-use
prohibition. This runner supports SoS-first when curated leads are supplied,
and uses county-scoped deterministic Serper as the autonomous fallback.
"""
...
COUNTY_RUNS: list[tuple[str, str | None]] = [
    ("me_cumberland_serper_queries.txt", "Cumberland"),
    ("me_york_serper_queries.txt", "York"),
    ("me_hancock_serper_queries.txt", "Hancock"),
    ("me_kennebec_serper_queries.txt", "Kennebec"),
    ("me_penobscot_serper_queries.txt", "Penobscot"),
    ("me_androscoggin_serper_queries.txt", "Androscoggin"),
    ("me_knox_serper_queries.txt", "Knox"),
    ("me_lincoln_serper_queries.txt", "Lincoln"),
    ("me_sagadahoc_serper_queries.txt", "Sagadahoc"),
    ("me_waldo_serper_queries.txt", "Waldo"),
    ("me_oxford_serper_queries.txt", "Oxford"),
    ("me_franklin_serper_queries.txt", "Franklin"),
    ("me_somerset_serper_queries.txt", "Somerset"),
    ("me_aroostook_serper_queries.txt", "Aroostook"),
    ("me_piscataquis_serper_queries.txt", "Piscataquis"),
    ("me_washington_serper_queries.txt", "Washington"),
]
