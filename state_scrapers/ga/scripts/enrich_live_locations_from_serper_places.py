#!/usr/bin/env python3
"""Enrich live Georgia HOA locations from Serper Places results.

This is the higher-yield GA map cleanup pass (adapted from KS). It queries
Serper Places for currently unmapped live GA HOAs, accepts only Georgia-
bounded/name-matching results, and writes records suitable for
`/admin/backfill-locations`.

Quality semantics:
- address: result has a street-like Georgia address.
- place_centroid: result is a Georgia subdivision/neighborhood/place centroid,
...
from state_scrapers.ga.scripts.enrich_live_locations_from_ocr import _fetch_render_jwt  # noqa: E402

BASE_URL = "https://hoaproxy.org"
STATE = "GA"
GA_BBOX = (30.3, 35.05, -85.7, -80.7)  # min_lat, max_lat, min_lon, max_lon
KS_BBOX = GA_BBOX  # backwards-compat name retained intentionally
SERPER_ENDPOINT = "https://google.serper.dev/places"
USER_AGENT = "HOAproxy GA Serper place cleanup/1.0 (admin@hoaproxy.org)"
...
def _in_state_bbox(lat: Any, lon: Any) -> bool:
    try:
        lat_f = float(lat)
        lon_f = float(lon)
    except (TypeError, ValueError):
        return False
    min_lat, max_lat, min_lon, max_lon = GA_BBOX
    return min_lat <= lat_f <= max_lat and min_lon <= lon_f <= max_lon


def _address_in_state(address: str | None) -> bool:
    return bool(
        re.search(r"\bGA\b|Georgia|\b3(?:0|1)\d{3}\b|\b39\d{3}\b", address or "", re.I)
    )
...
def _acceptance(hoa: dict[str, Any], place: dict[str, Any], min_score: float) -> dict[str, Any] | None:
    ...
    if not _in_state_bbox(place.get("latitude"), place.get("longitude")):
        return None
    if not _address_in_state(address):
        return None
...
def _queries(hoa: dict[str, Any]) -> list[str]:
    ...
    out = []
    if city:
        out.append(f'"{name}" {city} GA')
    out.append(f'"{name}" Georgia subdivision')
    return out
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("state_scrapers/ga/results/live_location_serper_places.json"))
