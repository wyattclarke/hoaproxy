#!/usr/bin/env python3
"""Enrich live Georgia HOA locations from OCR text and OSM/Nominatim.

This is a GA-specific cleanup tool for the live site, adapted from the KS
version. It reads live HOA summaries/documents, pulls already-indexed
searchable OCR text, extracts location clues, then proposes or posts
map-quality updates:

- OSM/Nominatim subdivision/neighborhood polygons when an extracted alias plus
  city/county resolves to a credible Georgia polygon.
- Census ZCTA centroids when document ZIP evidence is repeated and no polygon
  is found.

The script is intentionally conservative. It refuses out-of-Georgia
candidates, does not post city-only points, and defaults to dry-run.
"""
...
STATE = "GA"
STATE_NAME = "Georgia"
GA_BBOX = (30.3, 35.05, -85.7, -80.7)  # min_lat, max_lat, min_lon, max_lon
KS_BBOX = GA_BBOX  # backwards-compat alias for helpers
DEFAULT_USER_AGENT = "HOAproxy GA OCR location cleanup/1.0 (admin@hoaproxy.org)"
ZCTA_URL = (
...
STATE_RE = re.compile(
    r"\b(Georgia|Florida|Alabama|Tennessee|South Carolina|North Carolina)\b",
    re.I,
)
# Georgia ZIP codes: 30000-31999 and 39800-39999
ZIP_RE = re.compile(r"\b(3(?:0\d{3}|1\d{3}|98\d{2}|99\d{2}))(?:-\d{4})?\b")
COUNTY_RE = re.compile(r"\b([A-Z][A-Za-z .'-]+?)\s+County,\s+Georgia\b")
CITY_RE_PATTERNS = [
    re.compile(r"\bCity\s+of\s+([A-Z][A-Za-z .'-]+?),\s+Georgia\b"),
    re.compile(r"\b([A-Z][A-Za-z .'-]+?),\s+Georgia\b"),
]
ALIAS_PATTERNS = [
    re.compile(
...
def _in_state(lat: float | None, lon: float | None) -> bool:
...
def _extract_clues(hoa: dict[str, Any], text: str) -> dict[str, Any]:
    ...
    for pattern in CITY_RE_PATTERNS:
        for match in pattern.finditer(text):
            ...
            if city and city.casefold() not in {"state", "county", "georgia"}:
                cities.append(city)
...
def _nominatim_queries(clues: dict[str, Any]) -> list[str]:
    ...
    for alias in aliases[:5]:
        for city in cities[:2]:
            queries.append(f"{alias}, {city}, Georgia")
        for county in counties[:2]:
            queries.append(f"{alias}, {county} County, Georgia")
        queries.append(f"{alias}, Georgia")
    out: list[str] = []
    seen: set[str] = set()
...
def _is_state_result(result: dict[str, Any]) -> bool:
    address = result.get("address") if isinstance(result.get("address"), dict) else {}
    display = str(result.get("display_name") or "").casefold()
    return str(address.get("state") or "").casefold() == "georgia" or ", georgia," in display


def _select_polygon(results: Any, clues: dict[str, Any]) -> dict[str, Any] | None:
    if not isinstance(results, list):
        return None
    selected = prepared._select_nominatim_geometry([r for r in results if _is_state_result(r)])
    if not selected:
        return None
    lat = selected.get("latitude")
    lon = selected.get("longitude")
    if not _in_state(lat, lon):
        return None
...
def build_records(args: argparse.Namespace) -> dict[str, Any]:
    ...
    for hoa in summaries:
        ...
        if args.require_state_text and clues["state_counts"]:
            ga = int(clues["state_counts"].get("Georgia") or 0)
            other = sum(v for k, v in clues["state_counts"].items() if k != "Georgia")
            if other > ga and not hoa.get("city"):
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, default=Path("state_scrapers/ga/results/live_location_ocr_enrichment.json"))
    parser.add_argument("--nominatim-cache", type=Path, default=Path("state_scrapers/ga/cache/nominatim_ocr_location_cache.json"))
    ...
    parser.add_argument("--require-state-text", dest="require_state_text", action="store_true", default=True)
    parser.add_argument("--allow-state-conflict", dest="require_state_text", action="store_false")
