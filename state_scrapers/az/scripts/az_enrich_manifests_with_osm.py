...
Usage:
    ...
    python scripts/az_enrich_manifests_with_osm.py [--dry-run] [--limit N] [--county COUNTY_SLUG]

Strategy:
    For each manifest NOT already at geometry.source == "osm-place":
    1. Parse candidate place names from the HOA name.
    2. Query Nominatim with each candidate + county + "Arizona, USA".
    3. Accept the first result with class in {place,boundary,landuse} and
       address.state in ("Arizona", "AZ") and roughly matching county.
...
Audit log: data/az_osm_enrichment_audit.jsonl
...
AUDIT_JSONL = REPO_ROOT / "data" / "az_osm_enrichment_audit.jsonl"
...
def main() -> None:
    ...
    for i, uri in enumerate(manifest_uris):
        # Parse path: gs://bucket/v1/AZ/{county_slug}/{hoa_slug}/manifest.json
