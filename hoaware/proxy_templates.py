...
def render_proxy_form(
    ...
    *,
    ...
    """Render a proxy form as HTML for the given jurisdiction.
    ...
    # Build statutory citation from the first available rule
    ...
    # Compute expiry if not explicitly provided
    if not expires_at:
        expires_at = _compute_expiry(validity_duration, meeting_date)

    # Determine if undirected fallback allows discretionary voting
    undirected_fallback = True  # default
    if directed_option and "must be directed" in directed_option.lower():
        undirected_fallback = False

    # Electronic assignment note
    ...
    # Parse voting instructions from JSON if string
